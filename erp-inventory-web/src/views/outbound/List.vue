    ...
    <el-card>
      <template #header>
        <div class="card-header">
          ...
          <div class="header-right">
            ...
            <el-button v-if="hasPermission('outbound:create')" type="primary" @click="handleAdd">
      ...
      <el-table :data="tableData" border stripe v-loading="loading" :row-class-name="getRowClassName">
        ...
        <el-table-column label="操作" width="200" fixed="right">
          <template #default="{ row }">
            <el-button
              v-if="row.status === 0 && hasPermission('outbound:audit')"
              type="success"
              size="small"
            ...
            <el-button v-if="hasPermission('outbound:delete')" type="danger" size="small" @click="handleDelete(row)">
              删除
            </el-button>
          </template>
        </el-table-column>
...
const handleExport = async () => {
  try {
    await downloadWithAuth('/api/export/outbound', 'outbound-orders.xlsx')
...
const handleAudit = async (row) => {
  try {
    ...
    const res = await outboundApi.audit(row.id)
    if (res.code === 200) {
...
const handleDelete = async (row) => {
  try {
    ...
    const res = await outboundApi.delete(row.id)
    if (res.code === 200) {
